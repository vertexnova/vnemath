...
#
# This script builds VneMath for Linux
#==============================================================================
...
usage() {
  echo "Usage: $0 [-t <build_type>] [-c <compiler>] [-a <action>] [-clean] [-j <jobs>]"
  echo "Options:"
...
if [ "$COMPILER" = "gcc" ]; then
  CC_COMPILER="gcc"
  CXX_COMPILER="g++"
  COMPILER_VERSION=$(gcc --version | head -n 1 | awk '{print $NF}')
...
# Build CMake command
build_cmake_command() {
  local base_cmd="cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=$CC_COMPILER -DCMAKE_CXX_COMPILER=$CXX_COMPILER -DBUILD_TESTS=ON"
